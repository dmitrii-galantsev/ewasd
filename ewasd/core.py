...
def _is_nested_repo_root(path: Path) -> bool:
    """True if *path* is the root of a different repository.

    Submodules and linked worktrees mark their root with a ``.git`` file, plain
    clones with a ``.git`` directory; either way the tree below belongs to
    another repo.
    """
...
def _consolidate_monorepo_gitignores(git_root: Path) -> Path:
    ...
    try:
        ...
        #
        # Nested repositories are pruned too. Their entries are written relative
        # to *their* root, so importing them yields paths that are wrong here --
        # and descending into them is what made this walk cost seconds on large
        # trees (1.2M -> 150k entries on a TheRock checkout).
        for root, dirnames, filenames in os.walk(git_root):
            dirnames[:] = [
                d
                for d in dirnames
                if d not in IGNORED_VCS_DIRS and not _is_nested_repo_root(Path(root) / d)
            ]
        ...
        pass
    out = git_root / GITIGNORE_FILENAME
    header = (
        "# Auto-generated by ewasd\n"
        "# Editor configuration files (consolidated from every ewasd project in this repo)\n"
    )
...
def _in_linked_worktree(cwd: Path) -> bool:
    """True if *cwd* sits in a linked worktree rather than the main checkout.

    Linked worktrees share the main checkout's ``.git/config``, so an ordinary
    ``git config`` write from one silently reconfigures every other worktree.
    """
    git_dir = _git_output(cwd, "rev-parse", "--absolute-git-dir")
...
def _enable_worktree_config(cwd: Path) -> bool:
    """Ensure extensions.worktreeConfig is on so per-worktree writes are possible."""
    if _git_output(cwd, "config", "extensions.worktreeConfig") == "true":
        return True
    # git requires core.bare / core.worktree to be relocated before this
    # extension is enabled; refuse rather than risk breaking the repository.
    blockers = [
        key
        for key in ("core.bare", "core.worktree")
        if (_git_output(cwd, "config", "--local", key) or "false") != "false"
    ]
    if blockers:
        warn(f"Cannot enable extensions.worktreeConfig while {' and '.join(blockers)} is set")
        return False
    try:
        subprocess.run(
        ...
        warn(f"Failed to enable extensions.worktreeConfig: {e}")
        return False
...
def _set_git_excludes(gitignore_path: Path, cwd: Path) -> None:
    ...
    Inside a linked worktree the write is scoped with ``--worktree`` so it does
    not overwrite the shared config that the main checkout relies on.
    """
    scope: list[str] = []
    if _in_linked_worktree(cwd):
        if not _enable_worktree_config(cwd):
            warn(
                "Leaving core.excludesFile unchanged to avoid clobbering the main checkout; "
                f"linked configs under {cwd} will show as untracked"
            )
            return
        scope = ["--worktree"]
    try:
        subprocess.run(
            ["git", "config", *scope, "core.excludesFile", str(gitignore_path.resolve())],
            cwd=str(cwd),
            check=True,
            capture_output=True,
        )
...
class Repo:
    ...
    def get_configs(self) -> list[str]:
        """Return top-level config entry names (files/dirs), excluding VCS dirs."""
    ...
    def link_directory(
        ...
        for child in sorted(src_dir.iterdir()):
            if child.name in IGNORED_VCS_DIRS:
                continue
            src_item, dst_item = src_dir / child.name, dst_dir / child.name
            rel_path = f"{rel_prefix}{child.name}"
            if src_item.is_dir():
                if dst_item.exists() and dst_item.is_dir():
                    linked.extend(
                        self.link_directory(src_item, dst_item, f"{rel_path}/", dry_run=dry_run)
                    )
                elif not dst_item.exists():
                    _do_link(src_item, dst_item, dry_run)
    ...
    def link_any(self, target_name: str, cwd: Path, *, dry_run: bool = False) -> list[str]:
        ...
        src, dst = self.link_dir / target_name, cwd / target_name
        if not src.exists():
    ...
    def update_gitignore(
        ...
        # Always consolidate at the git root. git honours exactly one
        # core.excludesFile, so consolidating only for subdirectories meant a
        # later run at the repo root replaced the merged file with a root-only
        # one, silently un-ignoring every subproject's symlinks.
        target_gitignore = (
            _consolidate_monorepo_gitignores(git_root) if git_root else local_gitignore
        )
...
def check_symlink_health(cwd: Path, repo: "Repo") -> list[SymlinkStatus]:
    ...
    if gi.is_symlink():
        results.append(
            SymlinkStatus(
                path=gi,
                target=Path(os.readlink(gi)),
                ok=gi.exists(),
                reason="ok" if gi.exists() else "broken",
            )
        )
    elif gi.exists():
...
def _check_entry(cwd: Path, link_dir: Path, name: str, results: list[SymlinkStatus]) -> None:
    ...
    if dst.is_symlink():
        ...
        if not dst.exists():
            ...
            results.append(SymlinkStatus(path=dst, target=target, ok=False, reason="wrong_target"))
        else:
...
class ConfigParser:
    ...
    def get_repo(self, name: str) -> Repo:
        ...
        return Repo(name=name, git_url=data["repo"], link_dir=self.workspace_dir / data["link_dir"])
...
def migrate_symlinks(ws: Path, old_ws: Path, target_dir: Path, dry_run: bool) -> tuple[int, int]:
